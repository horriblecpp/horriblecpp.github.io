...
function update(deltaTime) {
  if (currentScreen == 0) updateStartScreen(deltaTime);
  if (currentScreen == 1) updateGameWorld(deltaTime);
}
...
function updateBossGuy(deltaTime) {
  ...
  for (var i = 0; i < solidObjects.length; i++) {
    if (
      ...
      if (
        ...
        this.x - this.size < solidObjects[i].x + solidObjects[i].sizeX &&
        this.y - this.size - solidObjects[i].sizeY + this.velocity <
          solidObjects[i].y &&
      ...
      if (
        ...
        if (
          solidObjects[i].x + solidObjects[i].sizeX + this.size > leftX &&
          solidObjects[i].x + solidObjects[i].sizeX + this.size < this.x
        )
          leftX = solidObjects[i].x + solidObjects[i].sizeX + this.size;
        if (
          solidObjects[i].x - solidObjects[i].sizeX - this.size < rightX &&
          solidObjects[i].x - solidObjects[i].sizeX - this.size > this.x
        )
  ...
  if (gravityActive == 1) {
    ...
    if (this.velocity < -1) this.squishVel = 1;
    this.velocity = 0;
    this.jumps = 1;
  }
  ...
  if (this.size == 150) {
    if (this.x > stickman.x) left = true;
    else right = true;

    if (stickman.x + stickman.size * 0.5 > this.x - this.size * 0.5) {
      if (stickman.y > this.y + this.size * 0.5) this.size *= 0.975;
      else stickman.x = this.x - (this.size + stickman.size) * 0.5;
    }
  } else {
    this.size *= 0.975;
  }
  ...
  if (right) {
    if (this.x + deltaTime / 8 < rightX) this.x += deltaTime / 8;
    // this.walk += deltaTime / 8;
    this.rot += (1 - this.rot) * 0.025 * deltaTime;
  }
  if (left) {
    if (this.x - deltaTime / 8 > leftX) this.x -= deltaTime / 8;
    // this.walk -= deltaTime / 8;
    this.rot += (-1 - this.rot) * 0.025 * deltaTime;
  }
}
...
function updateStickMan(deltaTime) {
  ...
  for (var i = 0; i < mouseDown; i++) {
    if (mouseY[i] > window.innerHeight * 0.8) {
      if (!clickLeft) clickLeft = mouseX[i] < window.innerWidth * 0.2;
      if (!clickRight)
        clickRight =
          mouseX[i] > window.innerWidth * 0.2 &&
          mouseX[i] < window.innerWidth * 0.4;
      if (!clickUp) clickUp = mouseX[i] > window.innerWidth * 0.8;
    }
  }

  if (
    currentlyPressedKeys[38] ||
    clickUp ||
    currentlyPressedKeys[87] ||
    currentlyPressedKeys[17] ||
  ...
  if (currentlyPressedKeys[39] || clickRight || currentlyPressedKeys[68]) {
    if (this.x + deltaTime / 4 < rightX) this.x += deltaTime / 4;
    this.walk += deltaTime / 4;
    this.cameraFree = true;
  }

  if (currentlyPressedKeys[37] || clickLeft || currentlyPressedKeys[65]) {
    if (this.x - deltaTime / 4 > leftX) this.x -= deltaTime / 4;
    this.walk -= deltaTime / 4;
    this.cameraFree = true;
  }
  if (this.walk > 100) this.walk -= 100;
  if (this.walk < 0) this.walk += 100;
  ...
  if (this.x > 200) {
    ...
    if (this.x > 350) {
      ...
      if (gameWorldJumpAlpha > 10) gameWorldJumpAlpha = 10;
    }
  } else {
    gameWorldAlpha += deltaTime * 0.001;
    if (gameWorldAlpha > 5) gameWorldAlpha = 5;
  }
...
function updateGameWorld(deltaTime) {
  if (window.innerWidth > 0)
    cameraZ +=
      ((window.innerHeight / window.innerWidth + 1) * 400 - cameraZ) *
      0.001 *
      deltaTime;
  ...
  if (isMobile)
    ...
    drawString(
      ...
      20,
      gameWorldAlpha / 3,
      "Use Arrowkeys to move the stickman"
    );
  ...
  drawString(
    ...
    0,
    10,
    gameWorldEducationAlpha / 30,
    this.window.resume[1]
  );
  drawString(
    1200,
    -200,
    0,
    10,
    gameWorldEducationGoalAlpha / 30,
    this.window.resume[2]
  );
  drawString(
    1600,
    -200,
    0,
    10,
    gameWorldCourseWorkAlpha / 30,
    this.window.resume[3]
  );
  ...
  drawString(
    ...
    0,
    10,
    gameWorldExperienceAlpha / 30,
    this.window.resume[4]
  );
  drawString(
    2100,
    -550,
...
function initGameWorld() {
  ...
  for (var i = 0; i < 40; i++) {
    ...
    addGameObject(c);
    if (c.x < 100 && c.x > -100 && c.y > -100 && c.y < 100) c.x += 100;
  }
